...
class ZooAnimal {
    ...
    static var typesOfAnimals: [String] = Array(Set(zooAnimals.reduce([], {$0 + [$1.classification]}))).sorted()
...
""")
    ...
    static func filterImages(with sectionName: String) {
        ViewController.myDataSource = ZooAnimal.zooAnimals.filter{$0.classification == sectionName}
    }
}
